...
def leer_alumno_bd(mat):
    try:
        cursor=con.connection.cursor()
        sql="select * from alumno where matricula = {0}".format(mat)
        cursor.execute(sql)
...
def registrar_alumno():
    try:
        ...
        if alumno != None:
            ...
            cursor=con.connection.cursor()
            sql=""" INSERT INTO alumno(matricula, nombre, apaterno, amaterno, correo)
                VALUES({0},'{1}','{2}','{3}','{4}')""".format(request.json['matricula'],request.json['nombre'],request.json['apaterno'],
                request.json['amaterno'],request.json['correo'])
            cursor.execute(sql)
...
def modificar_alumno(mat):
    try:
        ...
        if alumno != None:
            cursor=con.connection.cursor()
            sql=""" UPDATE alumno SET nombre='{0}', apaterno='{1}', amaterno='{2}', correo='{3}' where matricula={4}""".format(
...
def eliminar_alumno(mat):
    try:
        ...
        if alumno != None:
            cursor=con.connection.cursor()
            sql=""" DELETE FROM alumno WHERE matricula={0}""".format(mat)
            cursor.execute(sql)
            con.connection.commit()
